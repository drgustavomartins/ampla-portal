...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (path === "/api/modules" && method === "GET") {
      const m = await db.select().from(modules).orderBy(modules.order);
      return json(res, m);
    }

    const moduleMatch = path.match(/^\/api\/modules\/(\d+)$/);
    if (moduleMatch && method === "GET") {
      const [mod] = await db.select().from(modules).where(eq(modules.id, parseInt(moduleMatch[1])));
    ...
    if (path === "/api/lessons" && method === "GET") {
      const l = await db.select().from(lessons).orderBy(lessons.order);
      return json(res, l);
    }

    const moduleLessonsMatch = path.match(/^\/api\/modules\/(\d+)\/lessons$/);
    if (moduleLessonsMatch && method === "GET") {
      const l = await db.select().from(lessons).where(eq(lessons.moduleId, parseInt(moduleLessonsMatch[1]))).orderBy(lessons.order);
      return json(res, l);
    }

    const lessonMatch = path.match(/^\/api\/lessons\/(\d+)$/);
    if (lessonMatch && method === "GET") {
      const [lesson] = await db.select().from(lessons).where(eq(lessons.id, parseInt(lessonMatch[1])));
    ...
    if (progressMatch && method === "GET") {
      const progress = await db.select().from(lessonProgress).where(eq(lessonProgress.userId, parseInt(progressMatch[1])));
    ...
    if (completeMatch && method === "POST") {
      ...
      if (existing) {
        const [updated] = await db.update(lessonProgress).set({ completed: true, completedAt: new Date().toISOString() }).where(eq(lessonProgress.id, existing.id)).returning();
        return json(res, updated);
      }
      const [p] = await db.insert(lessonProgress).values({ userId, lessonId, completed: true, completedAt: new Date().toISOString() }).returning();
      return json(res, p);
    }

    const incompleteMatch = path.match(/^\/api\/progress\/(\d+)\/lesson\/(\d+)\/incomplete$/);
    if (incompleteMatch && method === "POST") {
      await db.delete(lessonProgress).where(and(eq(lessonProgress.userId, parseInt(incompleteMatch[1])), eq(lessonProgress.lessonId, parseInt(incompleteMatch[2]))));
    ...
    if (path === "/api/admin/students/pending" && method === "GET") {
      const pending = await db.select().from(users).where(and(eq(users.role, "student"), eq(users.approved, false)));
    ...
    if (approveMatch && method === "POST") {
      ...
      const [plan] = user.planId ? await db.select().from(plans).where(eq(plans.id, user.planId)) : [null];
      const days = plan ? plan.durationDays : 90;
      const expires = new Date();
      expires.setDate(expires.getDate() + days);
      const [updated] = await db.update(users).set({ approved: true, accessExpiresAt: expires.toISOString() }).where(eq(users.id, id)).returning();
    ...
    if (revokeMatch && method === "POST") {
      const [updated] = await db.update(users).set({ approved: false, accessExpiresAt: null }).where(eq(users.id, parseInt(revokeMatch[1]))).returning();
    ...
    if (deleteStudentMatch && method === "DELETE") {
      await db.delete(users).where(eq(users.id, parseInt(deleteStudentMatch[1])));
      return json(res, { success: true });
    }

    const patchStudentMatch = path.match(/^\/api\/admin\/students\/(\d+)$/);
    if (patchStudentMatch && method === "PATCH") {
      const [updated] = await db.update(users).set(req.body).where(eq(users.id, parseInt(patchStudentMatch[1]))).returning();
      if (!updated) return json(res, { message: "Aluno não encontrado" }, 404);
      const { password, ...safe } = updated;
      return json(res, safe);
    }
    ...
    if (path === "/api/admin/modules" && method === "POST") {
      const [mod] = await db.insert(modules).values(req.body).returning();
      return json(res, mod);
    }

    const patchModuleMatch = path.match(/^\/api\/admin\/modules\/(\d+)$/);
    if (patchModuleMatch && method === "PATCH") {
      const [updated] = await db.update(modules).set(req.body).where(eq(modules.id, parseInt(patchModuleMatch[1]))).returning();
    ...
    if (deleteModuleMatch && method === "DELETE") {
      await db.delete(lessons).where(eq(lessons.moduleId, parseInt(deleteModuleMatch[1])));
      await db.delete(modules).where(eq(modules.id, parseInt(deleteModuleMatch[1])));
      return json(res, { success: true });
    }

    // ── ADMIN: Lessons ──
    if (path === "/api/admin/lessons" && method === "POST") {
      const [lesson] = await db.insert(lessons).values(req.body).returning();
      return json(res, lesson);
    }

    const patchLessonMatch = path.match(/^\/api\/admin\/lessons\/(\d+)$/);
    if (patchLessonMatch && method === "PATCH") {
      const [updated] = await db.update(lessons).set(req.body).where(eq(lessons.id, parseInt(patchLessonMatch[1]))).returning();
    ...
    if (deleteLessonMatch && method === "DELETE") {
      await db.delete(lessons).where(eq(lessons.id, parseInt(deleteLessonMatch[1])));
      return json(res, { success: true });
    }
