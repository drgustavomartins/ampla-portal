...
import { Check, Clock, Zap, Users, Video, Star, ArrowRight, CreditCard, ChevronLeft, Loader2 } from "lucide-react";
...
function PlanCard({
  ...
  return (
    <div className={`relative flex h-full flex-col rounded-2xl border transition-all duration-200 hover:scale-[1.01] ${
      isDestaque
      ...
      {isNegociacao ? (
        <div className="mt-auto space-y-2">
          ...
          <a
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#25D366] py-3 text-sm font-semibold text-white transition-all hover:bg-[#1ebe5d]"
          >
          ...
          <div className="mb-14">
            ...
            <div className={`grid items-stretch gap-6 ${
              ...
              {planosNegociacao.map((plan) => (
                <PlanCard key={plan.key} plan={plan} onPagar={handlePagar} isLoading={false} />
              ))}
            </div>
          </div>
        )}
